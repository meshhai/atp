...
- [x] `mix test` passes on the branch before behavior changes.
- [x] `mix test --cover` output is reviewed.
- [x] Lowest runtime and dispatcher coverage modules are identified.
- [x] Targeted gaps are listed in `.scratch/runtime-ops-hardening/progress.txt`.
- [x] Coverage work is scoped to production-meaningful runtime, dispatcher, session, and CLI edges.
